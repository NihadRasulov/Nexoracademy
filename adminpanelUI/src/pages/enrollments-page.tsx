...
export function EnrollmentsPage() {
  ...
  const listQuery = useQuery({
    ...
    queryFn: () => api.get<EnrollmentRow[]>("/api/admin/enrollments"),
  });

  const invalidate = () => queryClient.invalidateQueries({ queryKey: ["enrollments"] });

  const createMutation = useMutation({
    mutationFn: (body: Record<string, unknown>) => api.post("/api/admin/enrollments", body),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: Record<string, unknown> }) =>
      api.patch(`/api/admin/enrollments/${id}`, body),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete<void>(`/api/admin/enrollments/${id}`),
    onSuccess: () => {
  ...
  const cancelMutation = useMutation({
    mutationFn: ({ id, reason }: { id: string; reason: string }) =>
      api.post(`/api/admin/enrollments/${id}/cancel`, reason ? { reason } : {}),
    onSuccess: () => {
        ...
        rowActions={(row) => (
          <div className="flex justify-end gap-1">
            {row.status !== "CANCELLED" && row.status !== "COMPLETED" && (
              <Button variant="ghost" size="icon" onClick={() => setCancelling(row)} title="Ləğv et">
