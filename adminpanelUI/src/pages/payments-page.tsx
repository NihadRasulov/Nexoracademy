...
export function PaymentsPage() {
  ...
  const listQuery = useQuery({
    queryKey: ["payments"],
    queryFn: () => api.get<PaymentRow[]>("/api/admin/payments"),
  });

  const invalidate = () => queryClient.invalidateQueries({ queryKey: ["payments"] });

  const createMutation = useMutation({
    mutationFn: (body: Record<string, unknown>) => api.post("/api/admin/payments", body),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: Record<string, unknown> }) =>
      api.patch(`/api/admin/payments/${id}`, body),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete<void>(`/api/admin/payments/${id}`),
    onSuccess: () => {
  ...
  const captureMutation = useMutation({
    mutationFn: (id: string) => api.post(`/api/admin/payments/${id}/capture`),
    onSuccess: () => {
