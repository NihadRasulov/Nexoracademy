...
export function CourseInstructorsPage() {
  ...
  const listQuery = useQuery({
    ...
    queryFn: () => api.get<CourseInstructorRow[]>("/api/admin/course-instructors"),
  });

  const invalidate = () => queryClient.invalidateQueries({ queryKey: ["course-instructors"] });

  const createMutation = useMutation({
    mutationFn: () => api.post("/api/admin/course-instructors", { courseId, instructorId, role }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: () =>
      api.patch(`/api/admin/course-instructors/${courseId}/${instructorId}`, { courseId, instructorId, role }),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (row: CourseInstructorRow) =>
      api.delete<void>(`/api/admin/course-instructors/${row.courseId}/${row.instructorId}`),
    onSuccess: () => {
