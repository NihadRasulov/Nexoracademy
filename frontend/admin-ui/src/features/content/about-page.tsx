...
export function AboutPage() {
  ...
  useEffect(() => {
    ...
    setDocument(found); setTitle(found?.title ?? "Nexora Academy haqqında"); setBody(found?.body ?? ""); setImageUrl(String(found?.data?.heroImage ?? "")); setMission(String(found?.data?.mission ?? "")); setPublished(found?.published ?? true); setReady(true);
  }, [query.data, ready]);

  const saveMutation = useMutation({
    mutationFn: () => {
      const payload = { key: KEY, type: "PAGE", title: title.trim(), body: body.trim(), data: { ...(document?.data ?? {}), heroImage: imageUrl.trim(), mission: mission.trim() }, published, sortOrder: 10 };
