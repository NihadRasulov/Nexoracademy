...
export function AcademyPage() {
  ...
  const query = useQuery({ queryKey: ["cms-academy"], queryFn: () => api.get<CmsContentDocument[]>("/api/v1/content/cms-content") });
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (saved) => { setDocument(saved); void queryClient.invalidateQueries({ queryKey: ["cms-academy"] }); toast.success("Akademiya səhifəsi yadda saxlanıldı."); },
  ...
  return <div><PageHeader eyebrow="Məzmun studiyası" title="Akademiya" description="Akademiya səhifəsinin əsas mətnini, vizualını və göstəricilərini idarə edin." actions={<><Button variant="outline" nativeButton={false} render={<a href="/academy.html" target="_blank" rel="noreferrer" />}><ExternalLink className="size-4" /> Saytı aç</Button><Button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending}>{saveMutation.isPending ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />} Yadda saxla</Button></>} /><ErrorBanner error={query.error} />
