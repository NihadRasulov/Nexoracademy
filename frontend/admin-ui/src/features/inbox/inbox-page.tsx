...
import { Download, FileText, Mail, MessageSquareText, Search, Trash2 } from "lucide-react";
...
export function InboxPage() {
  ...
  return <div><PageHeader eyebrow="Gələnlər mərkəzi" title="Müraciətlər" description="Saytdan gələn başvurular, əlaqə mesajları və newsletter abunəçiləri." />
    ...
    <Tabs defaultValue="applications" className="gap-5">
      ...
      <TabsContent value="applications"><DataTable rows={filter(applications.data, ["fullname", "email", "phone"])} loading={applications.isLoading} rowKey={(row) => String(row.id)} columns={[
        { key: "fullname", label: "Ad soyad" }, { key: "email", label: "E-poçt" }, { key: "phone", label: "Telefon" },
        { key: "letter", label: "Məktub", className: "max-w-xs truncate" }, { key: "cvFilename", label: "CV", render: (row) => row.cvFilename ? <a className="inline-flex items-center gap-1.5 font-medium text-primary hover:underline" href={adminApiUrl(`/api/v1/applications/${row.id}/cv`)}><Download className="size-3.5" />{row.cvFilename}</a> : "-" },
      ...
      <TabsContent value="contacts"><DataTable rows={filter(contacts.data, ["fullName", "email", "phone", "message"])} loading={contacts.isLoading} rowKey={(row) => row.id} columns={[
